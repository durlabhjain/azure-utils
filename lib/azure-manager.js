...
const azureManager = {
    loadParameters: async (config) => {
        ...
        program
            ...
            .option('-p, --cdnProfile <name>', 'cdn profile name', config.cdnProfile)
            .option('-e, --cdnEndpoint <name>', 'cdn endpoint', config.cdnEndpoint)
            ...
            .option('--indexDocument <name>', 'index document', config.indexDocument || 'index.html')
    ...
    createPromptConfig: function (config) {
        return [
            ...
            },
            {
                ...
                dependsOn: ['cdnSite'],
                initial: function () {
                    return config.storageAccount || config.fqdn.replace(/[^0-9A-Za-z]/g, '00');
                },
                onState: ({ value, aborted }) => {
                    if (!aborted) {
    ...
    generateRecipies: async function ({ configManager, recipies, promptConfig, nonInteractive }) {
        ...
        await recipeManager.generate({ tags: configManager.config, run: !config.dryRun });
    }
};
